...
@class NSURLRequest;
@class RFSInitializationConfiguration;
...
@class RFSMatchFacesConfiguration;
@class RFSFaceSDKVersion;
...
typedef NS_ENUM(NSUInteger, RFSLivenessProcessStatus) {
    ...
    RFSLivenessProcessStatusRemoveOcclusion
...
NS_SWIFT_NAME(LivenessDelegate)
@protocol RFSLivenessDelegate <NSObject>
...
NS_SWIFT_NAME(FaceCaptureDelegate)
@protocol RFSFaceCaptureDelegate <NSObject>
...
typedef void (^RFSInitializationCompletion)(BOOL success, NSError * _Nullable error) NS_SWIFT_NAME(FaceInitializationCompletion);
...
@property(readonly, nonatomic, strong, nullable) RFSFaceSDKVersion *version;
...
@property(readwrite, nonatomic, weak, nullable) id<RFSLivenessProcessStatusDelegate> processStatusDelegate RFS_DEPRECATED(6.2, "Use livenessDelegate instead");

/// Delegate that notifies about the face capture process events.
@property(readwrite, nonatomic, weak, nullable) id<RFSFaceCaptureDelegate> faceCaptureDelegate;

/// Delegate that notifies about the liveness process events.
@property(readwrite, nonatomic, weak, nullable) id<RFSLivenessDelegate> livenessDelegate;
...
@property(readonly, nonatomic, assign, getter=isInitialized) BOOL initialized;
